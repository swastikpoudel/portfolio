    ...
    <section id="contact" className="py-24">
      <div className="max-w-2xl mx-auto px-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button type="submit" disabled={status === 'sending'} className="w-full py-4 bg-gray-900 text-white rounded-xl font-medium hover:bg-gray-800 transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
            {status === 'sending' ? (
          ...
          {status === 'error' && <p className="text-red-500 text-center font-medium">Failed to send. Please try again.</p>}
        </form>
      </div>
    </section>
  )
